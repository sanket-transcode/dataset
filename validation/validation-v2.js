...
function validator(rules, keys, payload) {
  ...
  function processRule(rule, isMatchedBranch = true) {
    ...
    for (const chunk of chunks) {
      if (chunk.if) {
        ...
        if (chunk.then) {
          processRule(chunk.then, conditionMet);
        } else if (!conditionMet && chunk.else) {
          processRule(chunk.else, false);
        }
        continue;
      }

      // Unconditional rule OR branch (then/else) being processed
  ...
  // Process top-level rules (treat as matched/unconditional)
  for (const rule of rules) {
    processRule(rule, true); // top-level rules are always active
  }
