WITH product_attrs AS (
  ...
  WHERE 
    pa.product_attribute_group_id = 9617 
    AND pa.language_code = 'en_IE' 
...
attr AS (
  SELECT 
    ...
    COALESCE(
      MAX(
        CASE WHEN at.language_code = 'en_IE' 
    ...
    COALESCE(
      JSON_AGG(
        ...
        ORDER BY 
          CASE WHEN at.language_code = 'en_IE' THEN 0 ELSE 1 END, 
          at.key
      ) FILTER (
        WHERE 
          at.id IS NOT NULL
...
WHERE 
  ...
  AND pa.channel_id IS NULL 
  AND pa.variant_id IS NULL 
  AND a.name NOT IN (: bypassAttributeNames) 
ORDER BY 
